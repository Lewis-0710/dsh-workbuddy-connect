	...
	*/
	const migrateOwnSettings = () => {
		const legacy = readLegacySections([
			...
			WORKBUDDY_AI_SETTINGS_NS,
			WORKBUDDY_QUOTA_SETTINGS_NS
		]);
		const seeded = {};
		for (const key of CONFIG_KEYS) {
	...
	const checkInScheduler = new CheckInScheduler({
		targets: runtimes.map((runtime) => ({
			...
			minuteOfDay: () => {
				return normalizeCheckInMinute(readField(current(), runtime.variant.id === CN_VARIANT.id ? "checkInMinuteCN" : "checkInMinuteAI") ?? 600);
			},
			onClaimed: () => {
