...
const DISABLED_MODELS_FIELD = z.array(z.string()).default([]).description("Disabled model IDs for this variant (empty by default)");
...
function createVariantRuntime(config, variant, current, identityOf) {
	...
	if (variant.id !== CN_VARIANT.id) {
		...
		if (config.disabledModelsAI !== void 0) catalog.setDisabledModels(config.disabledModelsAI);
	} else if (config.disabledModelsCN !== void 0) catalog.setDisabledModels(config.disabledModelsCN);
...
function apply(ctx, config) {
	...
	*/
	...
	const checkInScheduler = new CheckInScheduler({
		targets: runtimes.map((runtime) => ({
			...
			minuteOfDay: () => {
				const cfg = current();
				return normalizeCheckInMinute((runtime.variant.id === CN_VARIANT.id ? cfg.checkInMinuteCN : cfg.checkInMinuteAI) ?? 600);
			},
			onClaimed: () => {
	...
	*/
	...
	ctx.inject(["settings"], (settingsCtx) => {
		...
		const applyDisabledModels = (next) => {
			for (const runtime of runtimes) {
