...
`;
		...
		const en = {
			...
			autoCheckInCN: "WorkBuddy (China) daily auto check-in",
			autoCheckInAI: "WorkBuddy AI (International) daily auto check-in",
		...
		const zh = {
			...
			autoCheckInCN: "WorkBuddy（国内版）每日自动签到",
			autoCheckInAI: "WorkBuddy AI（国际版）每日自动签到",
				...
				*/
				function registerPluginSettings() {
					...
					ctx.slots.inject("settings.section", () => {
						if (ctx.slots.entries("settings.section").some((entry) => entry.options?.id === PLUGIN_SETTINGS_SECTION_ID)) return registerItem();
						try {
							...
							}, PluginSettingsSection), registerItem()];
						} catch {
							return registerItem();
						}
					});
