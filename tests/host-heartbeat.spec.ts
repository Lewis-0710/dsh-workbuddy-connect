...
describe('host heartbeat', () => {
  it('writes, reads, and clears a heartbeat under data dir', async () => {
    ...
    expect(workbuddyHostHeartbeatPath()).toBe(join(workbuddyStateDir(), WORKBUDDY_HOST_HEARTBEAT_FILENAME))
  ...
  it('detects a recycled PID as dead (registeredAt after this process started)', async () => {
    ...
    if (startAtMs === undefined) {
      // In constrained environments where child_process execution is forbidden,
      // processStartTimeMs may be undefined.
      return
    }
