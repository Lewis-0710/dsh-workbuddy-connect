#!/usr/bin/env bash
#==============================================================================
# sync.sh — dsh-workbuddy-connect 上游同步脚本
#==============================================================================
#
# 功能：
...
#   - 文件：sync.patch（定制修改的干净快照，已排除 sync.patch/sync.sh/lib/pnpm-lock.yaml）
#
# 同步策略：
#   ── 策略 1：Patch Apply（优先）──────────────────────────────
...
#     4. 若无冲突，直接基于最新上游应用定制修改、重新构建并更新 custom 分支
#
...
#
#   ── 收尾工作：
#     1. 重新生成干净的 sync.patch（自动排除 sync.patch、sync.sh、lib 与 pnpm-lock.yaml）
#     2. 运行自动化测试与类型检查（pnpm run check）
...
if ! git remote get-url upstream >/dev/null 2>&1; then
    ...
    echo "💡 请先运行: git remote add upstream https://github.com/corrinehu/dsh-workbuddy-connect.git"
    exit 1
fi
...
# Step 0: 创建安全临时备份目录
TMP_DIR="$(mktemp -d /tmp/dsh_workbuddy_sync_XXXXXX)"
cleanup() {
    rm -rf "$TMP_DIR"
}
...
if [ -s "$TMP_DIR/sync.patch" ]; then
    ...
    if git apply --check "$TMP_DIR/sync.patch" 2>/dev/null; then
        ...
        cp "$TMP_DIR/sync.sh" "$SCRIPT_DIR/sync.sh" 2>/dev/null || true
        chmod +x "$SCRIPT_DIR/sync.sh"
        if command -v pnpm >/dev/null 2>&1; then
            pnpm run build --quiet 2>/dev/null || pnpm run build
        fi
        git add -A
...
# Step 4: 重新生成干净的 sync.patch（排除 sync.patch、sync.sh、lib 与 pnpm-lock.yaml）
echo "[4/4] 重新生成干净的 sync.patch..."
git diff upstream/main...custom -- ':!sync.patch' ':!sync.sh' ':!lib' ':!pnpm-lock.yaml' > "$PATCH_FILE"
PATCH_LINES=$(wc -l < "$PATCH_FILE" | tr -d ' ')
echo "✅ sync.patch 生成完毕 (共 $PATCH_LINES 行，已排除构建产物、依赖锁文件与同步脚本自身)"
