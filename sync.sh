...
if ! git remote get-url upstream >/dev/null 2>&1; then
    ...
    echo "💡 请先运行: git remote add upstream https://github.com/masknull/dsh-workbuddy-connect.git"
    exit 1
fi
...
if [ "$SKIP_TEST" = false ]; then
    if command -v pnpm >/dev/null 2>&1; then
        ...
        echo "🔍 正在运行项目构建与测试验证..."
        pnpm run build
        pnpm vitest run tests/client-fallback.spec.ts tests/slot-registration.spec.ts tests/version.spec.ts || true
    fi
fi
